...
describe('permalinks', () => {
  ...
  it('builds and applies permalink paths through history.replaceState', () => {
    ...
    expect(getRoomPermalinkPath('AbC-DeF-GhI')).toBe('/room/abc-def-ghi');
    expect(getOfflineModePermalinkPath('vs_pc')).toBe('/mode/vs-pc');

    setRoomPath('abc-def-ghi');
    setOfflineModePath('play_and_pass');
    setLobbyPath();

    expect(replaceState).toHaveBeenNthCalledWith(1, {}, '', '/room/abc-def-ghi');
    expect(replaceState).toHaveBeenNthCalledWith(2, {}, '', '/mode/pass-and-play');
    expect(replaceState).toHaveBeenNthCalledWith(3, {}, '', '/');
  });
});
