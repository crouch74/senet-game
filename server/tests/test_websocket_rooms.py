...
def test_room_join_rejects_ur_game_type_mismatch(client):
    room_id = client.post("/api/match/create?game=ur").json()["room_id"]

    with client.websocket_connect(f"/api/match/{room_id}?game=senet") as websocket:
        assert websocket.receive_json() == {
            "type": "error",
            "message": "Room game type mismatch: expected senet, got ur",
        }
